...
export default {
  ...
  methods: {
    async getTotalNetworkInfo() {
      try {
        const total_power = await wallet.getTotalNetworkPower();
        const total_adam = await wallet.getTotalAmounts();

        this.simpData.total_power = total_power; // 全网算力
        this.simpData.total_adam = wallet.formatNum(total_adam, 8);


        // test get data
        const poolname = await wallet.getPoolName("0xe95877899E4DDC53F95a1A67373D126435398EaB");
        console.log("poolname::", poolname);

        const userinfo = await wallet.getUserInfo("0xe95877899E4DDC53F95a1A67373D126435398EaB");
        const { pledgePower, amount } = userinfo; //当前算力
        // 质押数量:UserInfo.amount

        const userProfitInfo = await wallet.getUserProfitInfo("0xe95877899E4DDC53F95a1A67373D126435398EaB");

        // :UserProfitInfo.release+ UserProfitInfo.release_xssf
        const { profit, release, release_xssf, lock } = userProfitInfo; //产币量

        console.log("可用余额", release_xssf + release);

        console.log("pledgePower", pledgePower);
    ...
    // async getTotalNetworkInfo () {
    //    try{
    //         const total_power = await getTotalNetworkPower()
    //         const total_adam = await getTotalAmounts()

    //         this.simpData.total_power = total_power
    //         this.simpData.total_adam = formatNum(total_adam, 8)
    //    }
    //    catch(e) {
    //        console.log('e:::', e)
    //    }
